...
class AddFeedbackView(APIView):
    ...
    def post(self, request):
        ...
        print(request.data)
        if serializer.is_valid():
            diagnostic_accuracy = serializer.validated_data['Diagnostic_Accuracy']
            user_experience = serializer.validated_data['User_Experience']
            likelihood_to_recommend = serializer.validated_data['Likelihood_to_recommend']
            ...
            feedbackObjs = db.callStore("add_feedback",(diagnostic_accuracy,user_experience,likelihood_to_recommend,comment,appointment_id,user_id,createdtime))
            return JsonResponse({
                'message': 'successful!',
